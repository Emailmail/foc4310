...
void foc_start_adc( AuroFOC *foc ){
    ...
    HAL_TIM_PWM_Start( foc->htim , TIM_CHANNEL_4);
}
...
void foc_update_adc( AuroFOC *foc , uint32_t uu , uint32_t uv , uint32_t uw){
    ...
    float Uv_ = ((float)((int)uv - (int)uv_offset)) *  0.001611328f;
	foc->Ia = foc->Ia * 0.2f + 3.2f * Uu_;//0.8f * (Uu_ * 2.857142857f);// (ADC_multiple * Sample_R);
	foc->Ib = foc->Ib * 0.2f + 3.2f * Uv_;//0.8f * (Uv_ * 2.857142857f);// (ADC_multiple * Sample_R);
	foc->Ic = (-(foc->Ia + foc->Ib));//foc->Ic * 0.2f + 0.8f * (-(foc->Ia + foc->Ib));
}
...
void foc_start_pwm( AuroFOC *foc ){
    ...
    HAL_TIMEx_PWMN_Start( foc->htim , TIM_CHANNEL_1 );
    HAL_TIMEx_PWMN_Start( foc->htim , TIM_CHANNEL_2 );
    HAL_TIMEx_PWMN_Start( foc->htim , TIM_CHANNEL_3 );
    ...
    __HAL_TIM_ENABLE_IT( foc->htim  ,TIM_IT_UPDATE ); //开启注入中断
}
...
void _foc_update_pwm( AuroFOC *foc , float tu , float tv , float tw ){
	...
	//调整PWM占空比
	foc->htim->Instance->CCR1 = (uint16_t)(tu * 4250);
    foc->htim->Instance->CCR2 = (uint16_t)(tv * 4250);
    foc->htim->Instance->CCR3 = (uint16_t)(tw * 4250);
}
